...
response = await fetch(`/api/get-zy-exam`);
isZYBuiltExam = true; // 标记为内置赵宇题库，全多选
} else {
const requestUrl = `/${filePath}?userId=${encodeURIComponent(userLocalId)}`;
response = await fetch(requestUrl);
isZYBuiltExam = false; // 外部json题库，自动判断单选多选
}
...
try {
    ...
    examStartTime = Date.now(); // 记录答题开始时间
