...
- 分式不等式2：
  $\frac{a_1^{m+1}}{b_1^m}+…+\frac{a_n^{m+1}}{b_n^m} \geq \frac{(a_1+…+a_n)^{m+1}}{(b_1+…+b_n)^m}$
  （$m>0$）
...
- 辅助角公式：
  $a\sin x + b\cos x = \sqrt{a^2+b^2}\sin(x+\varphi)$，其中$\sin\varphi = \frac{b}{\sqrt{a^2+b^2}}$，$\cos\varphi = \frac{a}{\sqrt{a^2+b^2}}$，$\tan\varphi = \frac{b}{a}$
...
- 双纽线：
  $r^2 = a^2\sin2\theta$，$r^2 = a^2\cos2\theta$
