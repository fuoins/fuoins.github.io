...
|代码块高亮|原生CSS（IDEA风格）|复刻IDEA默认代码样式，无需Prism.js等插件，纯CSS实现字体、配色、布局，符合开发者阅读习惯|
...
代码块完全复刻IDEA默认Light主题风格，通过纯CSS实现，核心匹配点包括：IDEA默认等宽字体、语法高亮配色（关键字、字符串、注释等）、轻微灰色背景、无明显边框、清晰的行高与缩进，同时保留响应式适配。

核心样式代码（对应`assets/css/main.css`）：

```css

/* 代码块容器：复刻IDEA Light主题基础样式 */
pre {
  background-color: #f8f8f8 !important; /* IDEA默认代码背景色 */
  border: none !important; /* IDEA无外边框，取消之前的边框 */
  border-radius: 0 !important; /* IDEA默认直角，取消圆角 */
  padding: 1.2rem !important;
  margin: 1.5rem 0 !important;
  overflow-x: auto !important;
  position: relative !important;
  font-family: 'JetBrains Mono', 'Consolas', 'Monaco', monospace !important; /* IDEA默认字体 */
}

/* 代码文本基础样式：匹配IDEA字号与行高 */
pre code {
  font-family: inherit !important; /* 继承IDEA字体 */
  font-size: 0.9rem !important; /* IDEA默认代码字号 */
  line-height: 1.7 !important; /* IDEA舒适行高，提升可读性 */
  white-space: pre !important;
}

/* IDEA风格语法高亮：按语言适配核心配色 */
/* 关键字：深紫色 */
code .keyword {
...
code .comment {
  ...
  font-style: italic !important; /* IDEA注释斜体 */
}
/* 函数/方法名：深蓝色 */
code .function {
...
pre::before {
  ...
  background-color: #e0e0e0 !important; /* IDEA浅灰风格 */
  color: #333333 !important; /* 深灰文字，适配浅背景 */
  font-size: 0.8rem;
  border-radius: 0 !important; /* 直角匹配IDEA */
  text-transform: uppercase;
}

/* 行内代码样式：同步IDEA风格 */
p code, li code {
  background-color: #f8f8f8 !important; /* 与代码块一致背景 */
  padding: 0.2rem 0.4rem !important;
  border-radius: 0 !important; /* 直角 */
  font-family: 'JetBrains Mono', 'Consolas', monospace !important; /* IDEA字体 */
  color: #7d5bbf !important; /* 匹配函数名颜色，醒目且统一 */
  border: 1px solid #e0e0e0 !important; /* 细边框，贴近IDEA行内代码样式 */
}

/* 移动端适配：保持IDEA风格的同时优化显示 */
...
print(f"排序后：{quick_sort(test_arr)}")  # 输出：[1, 1, 2, 3, 6, 8, 10]
```

补充说明：若未安装`JetBrains Mono`字体（IDEA默认字体），浏览器会自动降级为`Consolas`等系统默认等宽字体，不影响核心风格；若需完全匹配，可手动安装[JetBrains Mono字体](https://www.jetbrains.com/lp/mono/)（免费开源）。
...
|代码块字体未显示为JetBrains Mono|手动安装JetBrains Mono字体，或在CSS中调整字体优先级，增加系统默认等宽字体备选|
|移动端导航菜单不响应|检查JS代码是否正确获取元素，确认`id`是否与HTML中的`id`一致|
### 七、总结

本博客的核心设计理念是**无插件、轻量化、高可维护性**，通过原生Web技术和Jekyll原生功能，实现了一个功能完善、性能优异的技术博客。在开发过程中，我们解决了公式渲染、代码高亮（复刻IDEA风格）、列表序号出界等多个典型问题，积累了丰富的静态博客开发经验。
